...
static ID3D11Buffer* g_pVSConstantBuffer0 = nullptr; // 定数バッファ
static ID3D11Buffer* g_pVSConstantBuffer3 = nullptr; // register(b3)用
static ID3D11Buffer* g_pPSConstantBuffer0 = nullptr; // ピクセルシェーダの定数バッファb0
...
bool Shader3d_Initialize(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
	...
	if (!pDevice || !pContext) {
		MessageBox(nullptr, "デバイスがNULLです", "エラー", MB_OK); 
		return false;
	}
	...
	if (FAILED(hr)) {
		...
		MessageBox(nullptr, "VS作成失敗", "エラー", MB_OK);
		return false;
	}
	...
	if (FAILED(hr)) {
		...
		MessageBox(nullptr, "レイアウト作成失敗", "エラー", MB_OK);
		return false;
	}
	...
	hr = g_pDevice->CreateBuffer(&buffer_desc, nullptr, &g_pVSConstantBuffer0);
	if (FAILED(hr)) {
		char buf[256];
		sprintf_s(buf, "通常バッファの作成に失敗しました。HRESULT: 0x%08X", hr);
		MessageBox(nullptr, buf, "DirectXエラー", MB_OK);
		return false;
	}
	...
	buffer_desc_light.Usage = D3D11_USAGE_DEFAULT;

	hr = g_pDevice->CreateBuffer(&buffer_desc_light, nullptr, &g_pVSConstantBuffer3);
...
{
	if (g_pVSConstantBuffer0 == nullptr) {
		MessageBox(nullptr, "通常版のバッファがNULLです！", "デバッグ", MB_OK);
		return; // クラッシュを回避
	}
	// 定数バッファ格納用行列の構造体を定義
...
{
	if (!g_pVSConstantBuffer3) return;
